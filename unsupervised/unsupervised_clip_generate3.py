...
DISCOVERY_STATE_PATH = "results/discovery_state.pkl"
...
def main():
    ...
    if not os.path.exists(DISCOVERY_STATE_PATH):
        print(f"ERROR: '{DISCOVERY_STATE_PATH}' not found!")
        return
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            # Unsupervised clustering - SAME METHOD AS unsupervised.py
            # In training: Each player is a separate sample
            # In inference: Cluster each player, then take majority vote
            
            valid_mask = masks.cpu() > 0.5  # (batch, M, T) -> bool mask
            valid_embeds = embedding.cpu()[valid_mask]  # Extract valid player embeddings
            
            if len(valid_embeds) > 0:
                # Normalize EACH player embedding (same as training)
                valid_embeds_np = valid_embeds.numpy()
                valid_embeds_norm = normalize(valid_embeds_np, norm='l2')  # (N_players, 512)
                
                # PCA + Clustering for EACH player
                embeds_pca = pca.transform(valid_embeds_norm)  # (N_players, n_components)
                player_clusters = kmeans.predict(embeds_pca)  # (N_players,)
                
                # Majority vote: Most common cluster
                from collections import Counter
                cluster_counts = Counter(player_clusters)
                cluster_id = cluster_counts.most_common(1)[0][0]  # Most frequent cluster
                
                # Debug: Show cluster distribution
                cluster_dist = ", ".join([f"C{c}:{cnt}" for c, cnt in cluster_counts.most_common()])
            else:
                # Fallback if no valid players
                scene_feat = embedding.mean(dim=(0, 1, 2)).cpu().numpy()
                scene_feat_norm = normalize(scene_feat.reshape(1, -1), norm='l2')
                embed_pca = pca.transform(scene_feat_norm)
                cluster_id = kmeans.predict(embed_pca)[0]
                cluster_dist = f"C{cluster_id}:1 (fallback)"
            ...
            # DEBUG: Check mapping
            if discovered_label_id != true_label_id:
                print(f"\n[MISMATCH] cluster={cluster_id} -> label={discovered_label_id} ({discovered_name}) | True={true_label_id} ({true_name})")
                print(f"   Valid embeds shape: {valid_embeds.shape if len(valid_embeds) > 0 else 'EMPTY'}")
                print(f"   Cluster distribution: {cluster_dist}")

            # Video generation
            status_color = (0, 255, 0) if discovered_label_id == true_label_id else (0, 0, 255)
